...
def hd(pred, gt):
    if pred.sum() > 0 and gt.sum() > 0:
        hd95 = binary.hd95(pred, gt)# 对于Ranking使用的HD距离，平常的时候要用HD95距离
        print(hd95)
        return hd95
    else:
        return 0
...
def inference(fold):
    ...
    label_list = sorted(glob.glob(
        os.path.join(r"E:\PGN\ETFormer\DATASET\ETFormer_raw\ETFormer_raw_data\Task01_ACDC\labelsTs",'*nii.gz')))
    dest_file = r"E:\PGN\Results\ETFormer"
...
if __name__ == '__main__':
    ...
    # parser.add_argument("fold", default=1,help="fold name")
