...
from ETFormer.experiment_planning.summarize_plans import summarize_plans
...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              search_in=(ETFormer.__path__[0], "training", "network_training"),
                              base_module='ETFormer.training.network_training'):
    ...
    if task == 'Task001_ACDC':
        ...
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([16, 160, 160])  # Unetr里面全都是(16,160,160)
        ...
        plans['plans_per_stage'][Stage]['batch_size'] = 2
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([64, 128, 128])  # 64, 128, 128
    ...
    elif task == 'Task012_ACDC':
        plans['plans_per_stage'][Stage]['batch_size'] = 2
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([16, 160, 160])  # 64, 128, 128
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()


    elif task == 'Task013_COVID19':
        plans['plans_per_stage'][Stage]['batch_size'] = 2  # 小batch_size速度最快
        plans['plans_per_stage'][Stage]['patch_size'] = np.array(
            [64, 128, 128])  # 224, 224 对于Swin-Unet系列的2D网络，几乎都是224*224大小的
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
        ...
        plans['plans_per_stage'][Stage]['batch_size'] = 2
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([128, 128, 128])  # 128, 128, 128
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
        ...
        plans['plans_per_stage'][Stage]['batch_size'] = 2
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([32, 192, 192])# 32,192,192是UNETR_PP指定的
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()

    elif task == "Task010_Colon":
        plans['plans_per_stage'][Stage]['batch_size'] = 2
    ...
    print("###############################################")
    print("I am running the following ETFormer: %s" % network)
