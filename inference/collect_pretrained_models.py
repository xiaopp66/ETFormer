...
import shutil
from ETFormer.paths import default_cascade_trainer, default_plans_identifier, default_trainer, network_training_output_dir
...
def copy_pretrained_models_for_task(task_name: str, output_directory: str,
                                    ...
                                    ETFormer_trainer=default_trainer,
                                    ETFormer_trainer_cascade=default_cascade_trainer,
                                    plans_identifier=default_plans_identifier):
    trainer_output_dir = ETFormer_trainer + "__" + plans_identifier
    trainer_output_dir_cascade = ETFormer_trainer_cascade + "__" + plans_identifier
...
def export_one_task(taskname, models, output_folder, ETFormer_trainer=default_trainer,
                    ETFormer_trainer_cascade=default_cascade_trainer,
                    plans_identifier=default_plans_identifier):
    copy_pretrained_models_for_task(taskname, output_folder, models, ETFormer_trainer, ETFormer_trainer_cascade,
                                    plans_identifier)
    copy_ensembles(taskname, output_folder, models, (ETFormer_trainer, ETFormer_trainer_cascade), (plans_identifier,))
...
def export_pretrained_model(task_name: str, output_file: str,
    ...
    for m in models:
        ...
        if not isfile(join(expected_output_folder, "postprocessing.json")):
            if strict:
                raise RuntimeError('postprocessing.json missing. Run ETFormer_determine_postprocessing or disable strict')
            else:
    ...
    valid = []
    for s in subd:
        v = check_if_valid(s, models, (ETFormer_trainer, ETFormer_trainer_cascade), (plans_identifier))
        if v:
            valid.append(s)
    for v in valid:
...
def export_entry_point():
    ...
    parser = argparse.ArgumentParser(description="Use this script to export models to a zip file for sharing with "
                                                 ...
                                                 "for usage with ETFormer_download_pretrained_model_by_url, or share the "
                                                 "zip for usage with ETFormer_install_pretrained_model_from_zip")
    ...
    parser.add_argument('-pl', type=str, help='ETFormer plans identifier. Default: %s' % default_plans_identifier,
...
def export_for_paper():
    output_base = "/media/fabian/DeepLearningData/ETFormer_trained_models"
